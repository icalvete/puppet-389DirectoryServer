class 389ds::cluster (
  ...
  if $cluster {
    ...
    exec {'config_389ds_ha':
      command => "/usr/bin/ldapadd -h localhost -p 389 -D '${ldap_admin_user}' -w ${ldap_admin_pass} -f ${config_ha_file}",
      user    => 'root',
      unless  => "/usr/bin/ldapsearch -h localhost -p 389 -D '${ldap_admin_user}' -w ${ldap_admin_pass} -s sub -b cn=config '(cn=replication manager)' | grep numEntries",
      require => File[$config_ha_file],
      before  => Exec['enable_389ds_ha']
    }

    exec {'enable_389ds_ha':
      command => "/usr/bin/ldapadd -h localhost -p 389 -D '${ldap_admin_user}' -w ${ldap_admin_pass} -f ${enable_ha_file}",
      user    => 'root',
      onlyif  => "/usr/bin/ldapsearch -h ${cluster_peer} -p 389 -D '${ldap_admin_user}' -w ${ldap_admin_pass} -s sub -b cn=config '(cn=replication manager)' | grep numEntries",
